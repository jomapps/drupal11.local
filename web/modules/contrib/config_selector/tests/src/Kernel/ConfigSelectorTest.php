...
class ConfigSelectorTest extends KernelTestBase {
  ...
  public function testConfigSelector() {
    ...
    // to be created. This will cause config_test.dynamic.feature_a_four to
    // still be enabled and feature_a_one will be disabled.
    $module_installer->install(['config_selector_test_one']);
    $configs = \Drupal::entityTypeManager()->getStorage('config_test')->loadMultiple();
    $this->assertFalse($configs['feature_a_one']->status());
    $this->assertArrayNotHasKey('feature_a_two', $configs);
   ...
   */
  protected function assertLogMessages(array $messages = [], string $level = LogLevel::INFO): void {
   ...
   *   The messages we expect the Drupal message service to have. Pass in an
   *   empty array to assert no messages.
   * @param string $type
   *   (optional) The type of the expected messages. Defaults to 'status'.
   */
  protected function assertMessages(array $messages, string $type = 'status'): void {
