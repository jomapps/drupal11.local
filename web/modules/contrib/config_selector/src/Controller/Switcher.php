...
class Switcher extends ControllerBase {
   ...
   */
  public function select(FeatureInterface $config_selector_feature, string $config_entity_type, string $config_entity_id): RedirectResponse {
   ...
   */
  protected function getConfigEntity(FeatureInterface $config_selector_feature, string $config_entity_type, string $config_entity_id): ConfigEntityInterface|false {
    /** @var \Drupal\Core\Config\Entity\ConfigEntityInterface $config_entity */
    $config_entity = $this->entityTypeManager()->getStorage($config_entity_type)->load($config_entity_id);
    if (!$config_entity) {
      $this->messenger()->addWarning(
        $this->t('Configuration entity of type %type and ID $id does not exist.',
          [
    ...
    if ($config_entity->getThirdPartySetting('config_selector', 'feature') !== $config_selector_feature->id()) {
      $this->messenger()->addWarning(
        $this->t('Configuration entity %config_label is not part of the %feature_label feature.',
          [
            '%config_label' => $config_entity->label(),
            '%feature_label' => $config_selector_feature->label(),
          ]
        )
      );
      return FALSE;
